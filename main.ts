...
export default class md2html extends Plugin {
			...
			html = html
				.replace(/<h. data-heading=".*?" dir=/g, (match) => {
					...
					const link = match
						...
						.replace('" dir=', "");
					let anchor = link.replace(/\W+/g, "-").toLowerCase();
					while (anchor.charAt(anchor.length - 1) == "-") {
						anchor = (anchor.length > 1)
	...
	async getRendered(toc = false): Promise<string> {
	...
	async render(md: string): Promise<string> {
		...
		comp.load();
		const div = createDiv();
		await MarkdownRenderer.render(this.app, md, div, "", comp);
		const html = this.clean(div.innerHTML);
		comp.unload();
		return html;
	}
	...
	async onload() {
		...
		this.addCommand({
			...
			editorCallback: async () => {
				navigator.clipboard.writeText(await this.getRendered());
		...
		this.addCommand({
			...
			editorCallback: async (editor: Editor) => {
				...
				new Notice("selection html copied to the clipboard", 3500);
			},
		});
		//--- file commands
		this.addCommand({
		...
		this.addCommand({
			id: "md2html-doc-toc",
			name: "convert note to html with table of contents",
			editorCallback: async (editor: Editor) => {
				editor.setValue(await this.getRendered());
				new Notice("document converted to html", 3500);
			},
		...
		this.addCommand({
			...
			name: "usage & help",
			callback: async () => {
				const help = this.app.setting;
				help.open();
				help.openTabById("md2html");
			},
		});
